...
import UIKit
import Kingfisher
protocol SliderGalleryControllerDelegate :NSObjectProtocol {
    //获取数据源
    func galleryDataSource()->[String]
    //获取内部scrollerView的宽高尺寸
    func galleryScrollerViewSize()->CGSize
}
class ChangeViewVC: UIViewController,UIScrollViewDelegate {
    //代理对象
   weak var delegate : SliderGalleryControllerDelegate!
    ...
    func configurePlaceholder(){
        ...
        placeholderImage = UIImage(named: "ts")
    }
    ...
    func configureAutoScrollTimer() {
        ...
        autoScrollTimer = Timer.scheduledTimer(timeInterval: 3, target: self,
                                               selector: #selector(ChangeViewVC.letItScroll),
    ...
    func resetImageViewSource() {
        ...
        if self.currentIndex == 0 {
            self.leftImageView?.kf.setImage(with: ImageResource(downloadURL: URL(string: self.dataSource!.last!)!), placeholder: placeholderImage, options: nil, progressBlock: nil, completionHandler: nil)
            self.middleImageView?.kf.setImage(with: ImageResource(downloadURL: URL(string: self.dataSource!.first!)!), placeholder: placeholderImage, options: nil, progressBlock: nil, completionHandler: nil)
           
            let rightImageIndex = (self.dataSource?.count)!>1 ? 1 : 0 //保护
             self.rightImageView?.kf.setImage(with: ImageResource(downloadURL: URL(string: self.dataSource![rightImageIndex])!), placeholder: placeholderImage, options: nil, progressBlock: nil, completionHandler: nil)
        }
            //当前显示的是最后一张图片
        else if self.currentIndex == (self.dataSource?.count)! - 1 {
            self.leftImageView?.kf.setImage(with: ImageResource(downloadURL: URL(string: self.dataSource![self.currentIndex-1])!), placeholder: placeholderImage, options: nil, progressBlock: nil, completionHandler: nil)
            
             self.middleImageView?.kf.setImage(with: ImageResource(downloadURL: URL(string: self.dataSource!.last!)!), placeholder: placeholderImage, options: nil, progressBlock: nil, completionHandler: nil)
            
             self.rightImageView?.kf.setImage(with: ImageResource(downloadURL: URL(string: self.dataSource!.first!)!), placeholder: placeholderImage, options: nil, progressBlock: nil, completionHandler: nil)
        }
            //其他情况
        else{
            self.leftImageView?.kf.setImage(with: ImageResource(downloadURL: URL(string: self.dataSource![self.currentIndex-1])!), placeholder: placeholderImage, options: nil, progressBlock: nil, completionHandler: nil)
              ...
              self.middleImageView?.kf.setImage(with: ImageResource(downloadURL: URL(string: self.dataSource![self.currentIndex])!), placeholder: placeholderImage, options: nil, progressBlock: nil, completionHandler: nil)
          
             self.rightImageView?.kf.setImage(with: ImageResource(downloadURL: URL(string:self.dataSource![self.currentIndex+1])!), placeholder: placeholderImage, options: nil, progressBlock: nil, completionHandler: nil)
        }
    }
